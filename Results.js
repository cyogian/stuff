...
import { PDFFindController } from "pdfjs-dist/web/pdf_viewer";
  ...
  export default function Results(props) {
    ...
    const [highlights, setHighlights] = useState([]); // Array Of Annotations
    ...
    useEffect(() => {
  //     set page input on page switch through scroll or navigation buttons
      setPageInput(page);
    }, [page]);
    const bboxToHighlight = (obj) => {
  //     Stride BBOX object to react-pdf-highlighter higlight object
    ...
    const highlightToBbox = (position) => {
      // react-pdf-highlighter position object(onAreaSelection) to Stride BBOX object 
      const {
        pageNumber,
    ...
    const pdfHighlighter = useRef(null); // ref for current pdf highlighter object
    const scrollToHighlight = useCallback(() => {
      // scrollToHighlight callback 
    ...
    const onChangeHighlight = (obj) => {
      // runs whwn a new highlight/annotation is selected
  ...
  const onPageInputChange = (evt) => {
    if (totalPages >= evt.target.value >= 1) {
      document
        ...
        ?.scrollIntoView(); // jumping to new page by using data-page-number attribute selector
    } else {
      setPageInput(page);
    }
  };
  const [pdfDocument, setPdfDocument] = useState(null); // stores pdf document object
  const [pdfFindController, setPdfFindController] = useState(null); // stores pdf find controller object that controlls search (you can refer to pdfJS)
